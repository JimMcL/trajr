...
An R package for analysing 2-dimensional animal trajectories, which represent the movements of animals through space and time. It provides functions to easily calculate various quantitative measures of trajectories such as speed and straightness or tortuosity, as wel as providing a range of other operations such as resampling to fixed step lengths (_rediscretization_), determining turning angles and step lengths, and so on.

`Trajr` does not provide support for creating trajectories, a mechanism to extract trajectories from videos, it operates on existing trajectories which are sequences of (x, y, time) coordinates.
